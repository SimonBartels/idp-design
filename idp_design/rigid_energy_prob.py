...
from idp_design.rigid_utils import get_intra_distances, project_particles_back, get_coordinate_frame
...
def get_energy_fn(bonded_nbrs, base_unbonded_nbrs, domains, displacement_fn, use_gg=True):
    assert len(domains["domain_particle_indices"]) == 2
    AA = 20
    p0 = jax.nn.one_hot(domains["domain_sequences"][0], AA)
    p1 = jax.nn.one_hot(domains["domain_sequences"][-1], AA)

    original_intra_distances = jnp.concatenate(domains["domain_particle_initial_intra_distances"])
    ...
    def subterms_fn(R, pseq, unbonded_nbrs=base_unbonded_nbrs, debye_kappa=default_debye_kappa):
        ...
        """
        TODO: here I need to take R and
        1) add the artificial energy term to enforce the rigid distance
        2) project the three particles s.t. the original distances are preserved
        3) change R to contain the positions of all residues
        4) change psed to contain the "probabilities" of all residues (let's see how bad it is to treat everything as one-hot)
        """
        current_intra_distances = jnp.concatenate([get_intra_distances(R[idx:idx+3, :], displacement_fn) for idx in domains["domain_particle_indices"]])
        rigid_energy = jnp.sum(jnp.square(current_intra_distances - original_intra_distances))  # TODO: correct sign?

        back_projected_particles = [project_particles_back(R[idx:idx+3, :], domains["domain_particle_initial_intra_distances"][i], displacement_fn) for i, idx in enumerate(domains["domain_particle_indices"])]
        # TODO: I might have to use vmap?
        new_coordinate_frames = [get_coordinate_frame(back_projected_particles[i]) for i in range(len(back_projected_particles))]
        domain_coordinates = [jnp.linalg.solve(new_coordinate_frames[i], domain_coordinates) for i, domain_coordinates in enumerate(domains["domain_coordinates"])]
        # TODO: generalize the lines below
        assert len(domains["domain_particle_indices"]) == 2
        R = jnp.concatenate([domain_coordinates[0], R[3:-3, :], domain_coordinates[-1]], axis=0)
        ...
        total_energy = total_bonded_val + total_unbonded_val + rigid_energy
